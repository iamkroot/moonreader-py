...
def get_drive_service(config):
    ...
    if config["sa_json"]:
        # If we have a Service Account JSON in environment or a path from config.toml
...
@app.route(route="google_drive_webhook", methods=("POST",))
async def google_drive_webhook(req: func.HttpRequest) -> func.HttpResponse:
...
def refresh_channel_timer(myTimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.error(f"Failed to refresh channel via Timer: {e}")
